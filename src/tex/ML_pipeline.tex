...
as the addition of priors to the model training stage. Generalised stacking has been applied in several astrophysical problems. That is the case of \citet{2016MNRAS.460.3152Z}, \citet{2023A&A...671A..99E}, \citet{2022A&A...666A..87C}, and \citet{2023MNRAS.520.3529E}.
...
\begin{table}
  ...
  \tablefoot{Metrics obtained using the default probability threshold of $0.5$.\\
   ...
   Uncertainties correspond to the standard deviation of the metrics obtained in across all $10$ training folds (cf. Sect.~\ref{sec:model_selection}).}
  }
  \end{table}
...
\textbf{Values and uncertainties as in Table~\ref{table:fit_AGN_models}.}}
\end{table}
...
\tablefoot{\textbf{For display purposes, all metrics have been multiplied by $100$.\\
Uncertainties correspond to the standard deviation of the metrics obtained in across all $10$ folds.}}
\end{table}
...
\subsection{Random classification}\label{sec:random_classification}

\textbf{The results from the classification stages of our pipeline can be, initially, compared to a random selection of sources. This selection implies generating random predictions following the class distribution of the studied sample. Table~\ref{table:random_classification} presents the scores generated by using this technique. These values are the base from which any improvement can be assessed.}

\begin{table}
  \setlength{\tabcolsep}{2pt}
  \caption{Results of random selection of sources in different stages of pipeline to the labelled sources in the HETDEX and Stripe\,82 fields.} % title of Table
...
This work & 0.9271      & 0.8764    & 0.9400    & 0.9167 \\
\hline\\[0.5em]
\hline\hline
...
This work & 0.9063     & 0.5853    & 0.9415    & 0.8791 \\
\hline
\end{tabular}
}
\tablefoot{
...
\begin{figure}
  \centering
    ...
    \includegraphics[width=0.99\columnwidth]{figures/predicted_probs_band_num_HETDEX_test_sep_class.pdf}
  \caption{Evolution of predicted probabilities (top: probability to be AGN, middle: probability of AGN to be detected in radio) and redshift values for radio-detectable AGN (bottom panel) as a function of the number of observed bands in all sources in test set. \textbf{In top panel, sources have been predicted between those predicted to be AGN and galaxy. In middle panel, sources are divided between predicted AGN that are predicted to be detected in radio and those predicted to not have radio detection.} Background density plot\textbf{s} (following colour coding in colorbars) show location of predicted values. Overlaid boxplots display main statistics for each number of measured bands. Black rectangles encompass sources in second and third quartiles. Vertical lines show the place of sources from first and fourth quartiles. Orange lines represent median value of sample and dashed, green lines indicate their mean values. \textbf{Dashed, gray lines show PR thresholds for AGN-galaxy and radio detection classifications.} \textbf{Close to} each boxplot, the written values correspond to the number of sources considered to create each set of statistics.}
...
The case of radio detection classification is somewhat different. For \textbf{less} than $\mathbf{10}$ detected bands (i.e. $\mathtt{band\_num} > \mathbf{10}$), the \textbf{distribution of} predicted \textbf{probabilities, for both sub-classes,} tend to \textbf{keep stable.} \textbf{From said value, probabilities} increase. \textbf{This evolution might be a consequence of the inclusion of measurements from 2M and, in particular, Ks band. This band is associated with the presence of AGN... {\color{red} complete this part!}}
...
\mathbf{g - r} &>& -0.76\,,\\
\mathbf{g - r} &<& 1.8\,,\\
\mathbf{W1 - W2} &>& 0.227 \times \mathbf{(g - r)} + 0.43\,,
\end{eqnarray}
