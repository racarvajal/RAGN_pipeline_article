...
redshift_reg      = redshift_reg_full.named_steps['trained_model'].estimators_[1]
...
explainer_z       = fasttreeshap.TreeExplainer(redshift_reg, data=None, 
...
_ = gf.plot_shap_decision('Redshift prediction', 'catboost', shap_values_z, explainer_z, 
                          reduced_cols, ax1, 'identity', new_base_value=0.0, 
                          base_meta='Base', xlim=xlims_plt, highlight=filter_radio)
plt.savefig(paths.figures / 'SHAP/SHAP_decision_z_base_catboost_HETDEX_highz.pdf', bbox_inches='tight')
