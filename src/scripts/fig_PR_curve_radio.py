...
filter_train_test  = catalog_HETDEX_df.index.isin(train_test_idx) * filter_AGN
...
prec_train_test_cal, recall_train_test_cal, thresh_train_test_cal =\
                     precision_recall_curve(catalog_HETDEX_df.loc[filter_train_test, 'LOFAR_detect'],
                                            catalog_HETDEX_df.loc[filter_train_test, 'Prob_radio'],
                                            pos_label=1)
...
viz_train.plot(ax=ax1,       lw=3.5, c=cm.get_cmap(gv.cmap_bands)(0.0), alpha=0.6, label=f"Training    (AUC = {auc_pr_train_cal:.4f})")
viz_test.plot(ax=ax1,        lw=3.5, c=cm.get_cmap(gv.cmap_bands)(0.3), alpha=0.6, label=f"Test        (AUC = {auc_pr_test_cal:.4f})")
viz_train_test.plot(ax=ax1,  lw=3.5, c=cm.get_cmap(gv.cmap_bands)(0.5), alpha=0.6, label=f"Train+Test  (AUC = {auc_pr_train_test_cal:.4f})")
viz_calib.plot(ax=ax1,       lw=3.5, c=cm.get_cmap(gv.cmap_bands)(0.7), alpha=0.6, label=f"Calibration (AUC = {auc_pr_calib_cal:.4f})")
viz_val.plot(ax=ax1,         lw=3.5, c=cm.get_cmap(gv.cmap_bands)(0.9), alpha=0.6, label=f"Validation  (AUC = {auc_pr_validation_cal:.4f})")
...
no_skill_train_test = np.sum(catalog_HETDEX_df.loc[filter_train_test, 'LOFAR_detect'] == 1)  / len(catalog_HETDEX_df.loc[filter_train_test, 'LOFAR_detect'])
...
ax1.plot([0, 1], [no_skill_train,      no_skill_train],      ls='--', marker=None, c=cm.get_cmap(gv.cmap_bands)(0.0), alpha=0.8, lw=3.5)
ax1.plot([0, 1], [no_skill_test,       no_skill_test],       ls='--', marker=None, c=cm.get_cmap(gv.cmap_bands)(0.4), alpha=0.8, lw=3.5)
ax1.plot([0, 1], [no_skill_train_test, no_skill_train_test], ls='--', marker=None, c=cm.get_cmap(gv.cmap_bands)(0.6), alpha=0.8, lw=3.5)
ax1.plot([0, 1], [no_skill_calib,      no_skill_calib],      ls='--', marker=None, c=cm.get_cmap(gv.cmap_bands)(0.8), alpha=0.8, lw=3.5)
ax1.plot([0, 1], [no_skill_val,        no_skill_val],        ls='--', marker=None, c=cm.get_cmap(gv.cmap_bands)(1.0), alpha=0.8, lw=3.5)
