...
for count, band in enumerate(feats_2_use):
    ...
    axs[count].annotate(text=rf'$\mathrm{{HETDEX ~ N}} = {np.sum(filt_lims_HETDEX_ni):,}$'.replace(',','$\,$'), 
                        xy=(0.98, 0.9), xycoords='axes fraction', fontsize=16, 
                        ha='right', va='top', path_effects=pe2)
    axs[count].annotate(text=f'$\mathrm{{S82 ~ N}} = {np.sum(filt_lims_S82_ni):,}$'.replace(',','$\,$'), 
...
axs[len(feats_2_use) - 1].legend(handles=[HETDEX_patch, S82_patch], 
                                 loc=3, fontsize=14, handletextpad=0.3, 
